...
# Generate config/app.json (json format)
echo "Generating config/app.json..."
cat > "config/app.json" << 'EOF'
{
    ...
    "logging": {
      ...
      "prettyPrint": "ENV_BOOL_LOG_PRETTY"
    },
    "database": {
      "url": "${DATABASE_URL:-\"postgresql://localhost:5432/eaa\"}",
      "host": "${DB_HOST:-\"localhost\"}",
      "port": "ENV_VALUE_DB_PORT",
      "name": "${DB_NAME:-\"eaa\"}",
      "username": "${DB_USER:-\"eaa\"}",
      "password": "${DB_PASSWORD:-\"\"}"
    },
    "frontend": {
      "distPath": "${FRONTEND_DIST_PATH:-\"public\"}"
    },
    "cors": {
    ...
    "crawler": {
      "timeout": "ENV_VALUE_CRAWLER_TIMEOUT"
    },
    "audit": {
      "storeType": "${AUDIT_STORE_TYPE:-\"memory\"}"
    }
...
sed -i "s|\"ENV_VALUE_PORT\"|\$\{PORT:-3000\}|g" "config/app.json"
sed -i "s|\"ENV_BOOL_LOG_PRETTY\"|\$\{LOG_PRETTY:-false\}|g" "config/app.json"
sed -i "s|\"ENV_VALUE_DB_PORT\"|\$\{DB_PORT:-5432\}|g" "config/app.json"
sed -i "s|\"ENV_VALUE_CRAWLER_TIMEOUT\"|\$\{CRAWLER_TIMEOUT:-30000\}|g" "config/app.json"
